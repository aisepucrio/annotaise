...
export default function QuestionVizualizer({ question, answer, badge, required = false }: QuestionVizualizerProps) {
  ...
  return (
    <article className="py-3 first:pt-0 last:pb-0">
      <div className="not-prose flex items-center justify-between gap-3">
        ...
        {visualBadge ? <div className="shrink-0 flex items-center">{visualBadge}</div> : null}
      </div>

      <div className="mt-2 rounded-md bg-gray-50 px-3 py-2 text-sm text-metal-700 wrap-break-word whitespace-pre-wrap">{answer}</div>
    </article>
  );
}
