...
export type { BarItem, QuestionSummary, QuestionSummaryChart, SummarySectionGroup } from './summary-vizualizer-utils';
...
export function SummaryQuestionCard({
  ...
  if (showResponseCount) {
    metadataItems.push(`${summary.responseCount} ${t('labelings.create.summary.responsesCount')}`);
  }
  ...
  return (
    <article className="relative overflow-hidden bg-white px-4 py-3">
      <div className="space-y-3">
        {showSectionLabel ? <p className="text-[11px] uppercase tracking-wide text-blueberry-700">{summary.sectionLabel}</p> : null}
