...
export default function LanguageToggle({ collapsed = false }: LanguageToggleProps) {
  ...
  return (
    <div className={cn('flex w-full pt-6 px-2', collapsed ? 'flex-col items-center gap-2' : 'flex-row justify-between')}>
        ...
        return (
          ...
          >
            <span className="block leading-none">
              <Flag className={cn(collapsed ? 'h-4 w-6' : 'h-8 w-12', 'rounded-xs')} />
            </span>
          </button>
        );
      })}
