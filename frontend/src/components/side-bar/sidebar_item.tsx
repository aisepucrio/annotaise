...
export default function SidebarItem({
  ...
  const isValidColor =
    typeof hover_color === "string" &&
    Object.prototype.hasOwnProperty.call(COLORS, hover_color);
  const hoverClasses = isValidColor ? COLORS[hover_color as ColorKey] : "";
  const baseActive = isValidColor
    ? COLORS[hover_color as ColorKey]
    : COLORS.blue;
    ...
    <Link
      ...
      className={`
        ...
        ${
          isActive
            ? activeClasses
            : `bg-white text-blueberry-900 ${hoverClasses}`
        }
      `}
      title={label}
      aria-label={label}
