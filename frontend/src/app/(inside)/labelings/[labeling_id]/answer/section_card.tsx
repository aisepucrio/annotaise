...
export default function SectionCard({ section, payload, answers, onChange, t }: SectionCardProps) {
  const orderedElements = useMemo(() => [...section.elements].sort((a, b) => (a.order ?? 0) - (b.order ?? 0)), [section.elements]);
  const sectionTitle = section.title?.trim() ? section.title : t('answer.section.title');

  const totalQuestions = orderedElements.filter((element) => element.question_type !== 'context').length;
            ...
            block.type === 'context' ? (
              <div key={`context-${blockIndex}`} className="space-y-2">
                {block.elements.map((context, contextIndex) => (
                  <ContextRow key={context.id ?? contextIndex} element={context} payload={payload} t={t} />
                ))}
              </div>
            ) : (
