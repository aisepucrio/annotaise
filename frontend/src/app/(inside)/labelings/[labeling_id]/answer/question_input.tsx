...
import type { LabelingStructureElement, ElementDTO } from '@/modules/labelings/labelingsTypes';
...
export default function QuestionInput({ element, value, onChange, answers, onAnswerChange }: QuestionInputProps) {
  switch (element.question_type) {
    ...
    case 'multiple_choice': {
      const items = [...(element.multiple_choice_items ?? [])].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
      ...
      return (
        <div className="space-y-2">
          {items.map((item, index) => {
            ...
            const followUpKey = followUp ? `followup_${element.id}_${item.id ?? index}` : null;
                ...
                <div key={item.id ?? index}>
                  <div className="flex items-start gap-2 text-sm text-metal-900">
                    <Checkbox
                      ...
                      checked={isChecked}
                      onChange={() => {
                        const next = isChecked ? selectedList.filter((val) => val !== optionValue) : [...selectedList, optionValue];
                        onChange(next);
                      }}
                      checkedColor="var(--blueberry-500)"
          ...
          {items.length === 0 ? <p className="text-xs text-metal-700">Nenhuma opção disponível.</p> : null}
        </div>
      );
    }

    default:
      return <p className="text-xs text-metal-700">Tipo de pergunta não suportado: {element.question_type ?? 'desconhecido'}</p>;
  }
}
...
function FollowUpQuestionBlock({ followUp, answerKey, answers, onAnswerChange }: FollowUpQuestionBlockProps) {
  const followUpElement: LabelingStructureElement = {
    ...followUp,
    multiple_choice_items: (followUp.multiple_choice_items ?? []).map((item, i) => ({ ...item, id: i })),
    ...
    <div className="ml-8 mt-2 mb-1 rounded-lg border border-blueberry-700-25 bg-blue-50 p-3">
      {followUp.text && (
        <div className="mb-2 flex items-center gap-1 text-sm font-medium text-blueberry-700">
          <ReactMarkdown remarkPlugins={[remarkGfm]}>{followUp.text}</ReactMarkdown>
          {followUp.required && <span className="text-red-400">*</span>}
        </div>
      )}
      <QuestionInput element={followUpElement} value={currentValue} onChange={(val) => onAnswerChange(answerKey, val)} />
    </div>
  );
}
