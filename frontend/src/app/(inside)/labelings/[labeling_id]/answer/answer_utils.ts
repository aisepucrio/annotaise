...
function getFollowUpDefault(
  followUp: LabelingStructureElement["multiple_choice_items"][number]["follow_up_question"],
): unknown {
  if (!followUp) return "";
...
function validateFollowUps(
  ...
  const sortedItems = [...(element.multiple_choice_items ?? [])].sort(
